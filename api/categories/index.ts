...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  if (req.method === 'GET') {
    let rows: string[][]
    try {
      rows = await readRows('categories')
    } catch {
      await createSheetWithHeaders('categories', CATEGORY_HEADERS)
      return res.status(200).json(await seedCategories())
    }
    if (rows.length === 0) {
      return res.status(200).json(await seedCategories())
    }
  ...
  if (req.method === 'POST') {
    ...
    const rows = await readRows('categories')
    ...
    await appendRow('categories', categoryToRow(newCat))
